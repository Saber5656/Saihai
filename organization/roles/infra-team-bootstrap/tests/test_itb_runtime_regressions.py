    ...
    def test_codex_consumers_redact_nonzero_process_output(self) -> None:
        ...
        for consumer in ("agent_dispatch", "provider_activate"):
            with self.subTest(consumer=consumer), tempfile.TemporaryDirectory() as tmp:
                ...
                row = next(item for item in roster if item["agent_id"] == "tech-backend")
                self.assertEqual({k: output[k] for k in ('decision', 'reason')},
                                 {"decision": "block", "reason": "codex provider process failed"})
                if consumer == 'provider_activate':
                    self.assertEqual(output['evidence']['request_id'], 'req-process-failure')
                    self.assertTrue(Path(output['evidence']['transcript_path']).is_file())
