...
独立した worker / reviewer を同時に動かす場合、`contents-director` は各 task の依存関係が無いことを確認し、builder の `agent-dispatch-batch` へ渡す item に `independent: true` または `dependency: none` を明示する。同一 role への同時 dispatch、同一文章を競合して編集する作業、前段の調査結果を読まないと進めない作業は batch 化しない。
