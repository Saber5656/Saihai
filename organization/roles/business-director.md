...
| Output Agents | business worker/reviewer roles, then `teams-project-manager` via structured Completion Report |
...
独立した worker / reviewer を同時に動かす場合、`business-director` は各 task の依存関係が無いことを確認し、builder の `agent-dispatch-batch` へ渡す item に `independent: true` または `dependency: none` を明示する。同一 role への同時 dispatch、同じ論点を別結論で扱う作業、前段判断を読まないと進めない作業は batch 化しない。
