...
| Root | 用途 |
|---|---|
| `${AGENTS_VAULT_ROOT}` | 組織フロー、Task Detail、Index、Kanban、Report |
| `${USER_VAULT_ROOT}` | ユーザー Vault、作業差分 |
| `${SKILLS_REPO_ROOT}` | Skills と runtime |
| `${DOTFILES_ROOT}` | 共通エージェント設定 |
...
```bash
python3 ${SKILLS_ROOT}/infra-task-dispatcher/scripts/itd_monitor.py
```

検証用:

```bash
python3 ${SKILLS_ROOT}/infra-task-dispatcher/scripts/itd_monitor.py --dry-run
python3 ${SKILLS_ROOT}/infra-task-dispatcher/scripts/itd_monitor.py --force --report /tmp/itd-report.md --root /tmp/sample-repo
```
