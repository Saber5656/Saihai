...
description: 整形済み Gate Intake Envelope を正式 Task artifact に変換する責務境界を表す互換 Gate ロール。既定経路は GPF report から ITB builder の `gtc-scaffold` command が直接実行され、Task Detail、Task Index、Kanban、Project Manager Handoff、Organization Active Set、active-task 登録は LLM が手書きせず builder command の決定論的 artifact を正本にする。
...
ただし、Task Detail、Task Index、Kanban、Project Manager Handoff、Organization Active Set、Invocation Evidence 初期欄、Execution Preflight、active-task 登録は **LLM が手書きしない**。
...
| Organization Active Set / Invocation Evidence initial rows | builder command |
