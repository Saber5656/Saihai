...
独立した worker / reviewer を同時に動かす場合、`infra-director` は各 task の依存関係が無いことを確認し、builder の `agent-dispatch-batch` へ渡す item に `independent: true` または `dependency: none` を明示する。同一 role への同時 dispatch、同一 Vault ファイルを競合して更新する作業、前段同期結果を読まないと進めない作業は batch 化しない。
