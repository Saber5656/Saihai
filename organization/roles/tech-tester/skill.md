...
組織フローや Organization Active Set を検証する場合は、既存テストを削除せず、追加テストとして扱う。

- Gate / Infra が常時 active であること。
- Tech / Contents / Business は必要時のみ active 化されること。
- bridge、commit、save、Obsidian CLI などの道具スキルが organization active set に混入しないこと。
...
- intended model / effective model / requestId / sessionId / usage source の証跡不足、未呼び出し、誤呼び出し、モデル不一致を failure として扱うこと。
