...
updated: 2026-06-14
status: reference
purpose: 旧 Guardian Verdict 契約の参照。runtime execution agent としては使わず、Finalization Check を正本にする
team: gate
...
新規 task で `gate-task-guardian` inbox、role-report、provider turn、runtime worker を作ってはならない。
...
|---|---|
| `gate-task-guardian` を runtime / queue consumer として起動する | `finalization-check` command |
