...
| gate-task-assessor | gate | reference | false | false | anthropic | claude-haiku-4-5 | claude-sonnet-4-6, gpt-5.4 | agent | low | low | compatibility_only |  | 2026-06-13: runtime agent から外す。TPM の `team-completion-check` command が完了確認を代替し、この行は責務境界参照用に残す |
| gate-task-creator | gate | active | true | false | anthropic | claude-haiku-4-5 | claude-sonnet-4-6, gpt-5.4 | agent | low | low | lazy_activation |  | 2026-06-14: 既定入口は GPF report -> `gtc-scaffold` command -> TPM queue へ移行。GTC provider は legacy / recovery fallback のみ lazy activation し、SessionStart では起動しない |
| gate-task-evaluator | gate | active | true | true | anthropic | claude-sonnet-4-6 | gpt-5.4, claude-haiku-4-5 | agent | medium | medium | lazy_activation |  | 2026-06-13: TPM の `team-completion-check` 通過後に queue activation で起動する。要求充足、レビュー結果、検証結果、Git 差分の関係を横断判断するため |
| gate-task-guardian | gate | reference | false | false | anthropic | claude-haiku-4-5 | claude-sonnet-4-6, gpt-5.4 | agent | low | low | compatibility_only |  | 2026-06-13: runtime agent から外す。GTE final review と Vault final update 後の `finalization-check` command が最終確認を代替し、この行は責務境界参照用に残す |
| git-publisher | dev | active | true | false | anthropic | claude-sonnet-4-6 | gpt-5.4, claude-haiku-4-5 | agent | medium | medium | lazy_activation |  | GTE の `quality_ok` + Git Publication Manifest から queue activation され、commit / push / PR tool flow の順序制御と Publication Result 記録を行う |
