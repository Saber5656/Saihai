...
At work-order issuance, the builder resolves `step.role` against the executing
Saihai checkout's `organization/roles/<role>/skill.md`. Request checkouts,
context references, and environment overrides cannot select this source. The
reader rejects unsafe components, symlink directories/files, nonregular files,
missing or unreadable files, invalid UTF-8, empty or oversized content, and
identity changes during the read. Repository files do not inherit private-state
`0700`/`0600` requirements.

A single bounded read supplies both `role_contract` (the complete UTF-8 text,
including original line endings) and `role_definition_digest`
(`sha256:<64 lowercase hexadecimal digits>`). `role_definition_path` is the
canonical repository-relative path. All three required fields are included
before existing work-order signing and snapshot freezing. The full contract is
appended as one deterministic role block; the complete instruction must fit the
existing 65,536-byte UTF-8 limit. There is no truncation or contract summarization.
Role content refines the assigned task and does not grant tools, permission,
provider access, approval, or a different output schema.

Drain validates the exact expected instruction and role ID against the template
step. Existing `WorkOrderError` handling blocks invalid orders with
`waiting_human` / `work_order_invalid` before new order/snapshot artifacts are
written. Missing role files are reported as `role_definition_unavailable`.

Frozen consumers additionally validate the signed role fields, canonical path,
content digest, full-text block placement and uniqueness, and byte limit through
`role_definition.validate_role_binding()`. This pure helper performs no file or
template reads and uses fixed `RoleDefinitionError` reasons, translated to
`WorkOrderError` or `ScopedWorkerError` at the respective boundary. Existing
snapshot, signature, activation, run, step, and iteration checks remain required.
A later role-file change or removal cannot change an already frozen instruction.
Consumers neither migrate nor re-sign old orders: missing role fields fail with
`role_definition_binding_missing`, even when the old signature is valid.

This generation and frozen-consumer contract does not yet add the digest to
normalized provider evidence or its gate, nor to the owner-bound bridge
projection. Those remain separate Issue #110 units. It does not attest actual
provider compliance, transitively pin role references, or establish human
approval custody or formal role-dispatch provenance.


### Display fixture normalization

The checked-in `tests/fixtures/run-artifacts` trees are deterministic viewer/API
fixtures. Their generator normalizes digest strings, timestamps, signatures,
and state paths, and removes host signing keys. In particular, a normalized
`role_definition_digest` is synthetic and need not hash its displayed contract.
These portable display artifacts are not executable signed work orders and
cannot prove runtime legacy rejection, human approval, or custody. Runtime
regressions instead create coherent signed order/snapshot fixtures in isolated
temporary state and exercise the actual frozen consumer. Fixture freshness
continues comparing complete generated bytes; no role fields are excluded.
