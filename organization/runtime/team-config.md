...
| Team | Initial State |
|---|---|
| Gate | resident / metadata_ready / eager process |
...
Gate 系 role と `teams-project-manager` は必ず `resident_target: true`、`always_active: true`、`startup_profile: provider_cli` とする。Claude 指定の role は activation / Invocation Evidence で Claude provider 証跡を必須にし、Codex/OpenAI evidence で代替してはならない。

agent-dispatch は resident provider の会話 context 肥大を防ぐため、`task_id` が前回 dispatch から変わった場合、または `dispatch_context_turns` が `ITB_AGENT_DISPATCH_CONTEXT_RESET_EVERY` / `context_reset_every`（既定 8）以上になった場合に provider pane を `force_respawn` する。respawn 理由、turn count、last task は roster と Invocation Evidence に記録する。
...
Vault、Task Index、Kanban は複数 Organization Instance から触られる共有資源である。排他制御、重複検出、採番整合性は `gate-task-creator` と `infra-task-dispatcher` の責務とする。
