...
| 4 | `teams-project-manager` | Task Detail、routing fields、Project Manager Handoff、Organization Active Set | 主担当チーム、支援チーム、レビュー証跡要件、実行順序、Branch Plan、Active Set |
...
| `team_roster_recorded` | Yes | Organization Active Set / role-agent registry evidence が記録されている |
...
| `team_roster_recorded` | Yes | Organization Active Set が Task Detail または Project note に記録されている |
...
`SessionStart` は metadata-only、`Stop` は read-only final gate として `infra-team-bootstrap` が扱う。task intake は必ず `gate-prompt-formatter` に渡す。

## Organization Active Set Contract

Organization Active Set は、利用可能な role metadata と現タスクで実際に動く active set を分離して記録するための契約である。
metadata は provider 応答、CLI 実行、完了 evidence を意味しない。
...
| `agent_instance_id` | Yes | チャットセッションごとの実 agent instance ID |
| `organization_instance_id` | Yes | チャット単位の Organization Instance ID |
| `context_scope` | Yes | `session` |
| `chat_session_id` | Yes | 紐づくチャット/作業セッション ID |
| `project_id` | When available | 紐づく Project ID または Vault project slug |
| `lifecycle_status` | Yes | `metadata_ready` / `active` / `deferred` / `archived` / `failed` |
| `agent_id` | Yes | 正式ロール ID |
| `team` | Yes | `gate` / `tech` / `contents` / `business` / `infra` |
| `activation_status` | Yes | `metadata_ready` / `active` / `deferred` / `response_active` / `resetting` |
| `metadata_status` | Yes | `metadata_ready` など、role metadata の作成状態 |
...
| `effective_model` | When available | provider output / session log で確認した実モデル |
| `execution_mode` | Yes | `agent` / `codex` / `chat` / `long-run` |
| `queue_consumer` | Yes | CLI dispatch 対象の inbox を consume するか |
| `inbox_path` | Yes | queue root からの相対 inbox path |
| `report_dir` | Yes | queue root からの相対 report directory |
| `session_id` | When available | Claude output sessionId、Codex session id など |
| `last_request_id` | When available | 最後に確認した requestId |
| `usage_source` | Yes | `claude_print_json`、`codex_exec` など |
| `active_for_task` | When active | 現在担当中の Task ID |
| `last_reset_at` | When reset | タスク切替時のリセット時刻 |
| `last_seen_at` | When available | 最後に evidence を確認した時刻 |
| `notes` | No | fallback、alias 解決、障害メモ |

Always active は運行上の core role を指し、provider response readiness を意味しない。on-call role は registry / model-registry に従い、必要時に active 化する。
...
`gate-task-assessor` と `gate-task-guardian` は旧フロー互換の参照 role として保持するが、新規 task の execution role / queue consumer / provider turn としては使わない。assessor 相当は `team-completion-check` command、guardian 相当は `finalization-check` / `final-transport-render-check` command が担う。

`infra-task-dispatcher` と `infra-local-qa` は Infrastructure の on-call role として扱い、自動巡回、状態同期、Vault QA が必要な時だけ active 化する。Tech / Contents / Business も TPM または各 director がタスクごとに active 化する。bridge、commit、git-publisher、push、git-workspace-prep、save、Obsidian CLI などの道具スキルは active set に入れない。
...
| role_id | agent_instance_id | organization_instance_id | context_scope | chat_session_id | project_id | lifecycle_status | agent_id | team | activation_status | metadata_status | tool_sidecar_status | response_status | always_active | provider | intended_model | effective_model | execution_mode | queue_consumer | inbox_path | report_dir | session_id | last_request_id | usage_source | active_for_task | last_reset_at | last_seen_at | notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
Goal Tracking は、Codex runtime の `goal` を親タスクの運行看板として扱い、Director / Gate / PM の組織フローに接続するための契約である。`goal` は作業主体ではなく、実作業は Active Set に載った role / team member が担当する。
...
| `delegation_evidence_required` | Yes | 実作業を role / team member へ dispatch した証跡が必要 |
...
| Organization Active Set | Yes | metadata / activation / response evidence、モデル証跡の初期欄 |
| Active Set | Yes | Gate core / Infra core、on-call role、タスク別 active set |
...
| Organization Active Set / Invocation Evidence / Queue Evidence がある | Yes |
| Gate core / Infra core、on-call role、道具スキルの一時実行が区別して記録されている | Yes |
