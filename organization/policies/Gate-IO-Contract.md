...
Completion Gate の機械検証用チェーン、pre-final 必須 section、main-agent 禁止 role 集合は `${AGENT_TEAMS_VIEWER_ROOT}/organization/runtime/infra-team-bootstrap/config/completion-chain.yaml` を正本とする。このノート内の flow 表は人間向け説明であり、ITB builder / Gate role は config を参照する。
