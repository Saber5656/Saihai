...
Completion Gate の機械検証用チェーン、pre-final 必須 section、main-agent 禁止 role 集合は `/Users/takagiyasushi/skills-repo/skills/infra-team-bootstrap/config/completion-chain.yaml` を正本とする。このノート内の図表は説明用であり、ITB builder / Gate role は config を参照する。
...
|---|---|---|
| `SessionStart` | `infra-team-bootstrap` | Resident Roster bootstrap |
| `SessionResume` | `infra-team-bootstrap` | Roster check and partial recovery |
| `PromptSubmit` | `gate-prompt-formatter` | Gate Intake Envelope |
| `SessionArchive` / `SessionClose` | `infra-team-bootstrap` shutdown mode | Handoff summary、resident shutdown、archived roster |

ユーザー発話は常に `gate-prompt-formatter` へ渡す。`チームを起動して` という発話も ITB 直通にしない。GPF が動く時点で Organization Instance / Resident Roster が存在しない場合は、GPF が起動を代行せず `bootstrap_missing` policy violation として停止する。

## Role-Agent Queue Operating Model

Shogun 型組織運用では、メインエージェントがサブエージェントを内側で再帰的に呼び出すのではなく、各 role agent が共通 queue を介して独立に作業する。

| Layer | Source of Truth | Rule |
|---|---|---|
| Role definition | 各 `SKILL.md` | 責務、Flow Contract、入力、出力、禁止事項を定義する |
| Team membership | `skills/infra-team-bootstrap/config/role-agent-registry.yaml` | Organization Instance で起動・待機対象になる role を定義する |
| Model / provider | `skills/infra-team-bootstrap/references/model-registry.md` | `intended_model`、provider、execution mode を定義する |
| Runtime queue | `<ITB_STATE_ROOT>/<session>/queue/` | inbox、task payload、report、lock を保持する。git 管理対象にしない |
| Runtime worker | `role-agent-worker` | pending message を claim し、report YAML と provider evidence を書く |
...
```text
UserPromptSubmit
  -> ITB prompt-preflight
  -> queue/inbox/gate-prompt-formatter.yaml
  -> gate-prompt-formatter role-agent-worker
...
`nudge` は、queue に message が入ったことを resident process へ知らせる補助信号であり、唯一の配送保証ではない。nudge が失敗しても queue write が成功していれば作業は失われない。結果は `queued_nudge_unconfirmed` として記録し、silent success にしない。
...
| Independent process | role agent は tmux / CLI adapter / worker loop など独立 process として扱う |
...
| Model self-report is not evidence | 生成本文内のモデル名ではなく、CLI header、transcript、session log を `effective_model` の証跡にする |
| Skill is not roster | `tech-*`、`contents-*` などのメンバーを Skill 直下に置く必要はあるが、常駐チーム構成は Skill 一覧ではなく registry で管理する |

Claude entry は tmux resident interactive session を既定とし、通常運用で `claude -p` を resident member 起動に使わない。Codex entry は Codex App / Codex CLI を入口にでき、OpenAI 系 member の実行は `codex exec` または同等の CLI adapter evidence として記録する。
...
| Execution owner | 実作業は resident agent / team member へ dispatch する |
...
```text
GPF -> gtc-scaffold / GTC fallback -> optional create_goal(parent task) -> TPM -> Director -> resident member dispatch -> team-completion-check -> GTE -> git publication / Vault final update -> finalization-check -> final transport render check -> main transport renderer -> update_goal complete
```
...
## Resident Organization Roster

組織ロールは作業セッション内で resident として起動済みであることを基本にする。resident は「そのロールが作業セッションに存在し、必要なときに呼べる状態」を指し、すべての resident が常に発言・作業することは意味しない。

| 状態 | 意味 |
|---|---|
| `resident` | Organization Instance 内で起動済み、または roster 上で起動対象として管理されている |
| `metadata_ready` | roster metadata が作成済み。provider 応答済みではない |
| `process_ready` | resident worker / provider CLI process が起動確認済み。provider response evidence とは別 |
| `response_active` | provider session / request / effective model / usage evidence 付きで応答済み |
| `active` | 現タスクで判断、作業、レビュー、同期、完了確認を担当する論理状態 |
| `idle` | resident だが現タスクでは発言・作業しない |
| `resetting` | タスク切替のため要約保存と `/clear` / `/compact` / `/new` を実施中 |
| `unavailable` | 起動失敗、モデル不一致、session 不明などで使えない |

Gate core と Infra core は全タスク横断の運行責務を持つ。ただし `always_active` は provider response ready を意味しない。process readiness、provider readiness、response evidence は別フィールドで管理する。
...
`gate-task-assessor` と `gate-task-guardian` は旧フロー互換の参照ロールであり、新規 task の resident / queue consumer / provider turn として起動しない。assessor 相当の確認は TPM が `team-completion-check` command evidence を正本に行い、guardian 相当の最終確認は Vault final update 後の `finalization-check` command と `final-transport-render-check` が担う。

`infra-task-dispatcher` と `infra-local-qa` は Infrastructure の on-call / lazy role として扱い、自動巡回、状態同期、Vault QA が必要なときだけ active 化する。Tech、Contents、Business も resident candidate だが、TPM または各 director がタスクごとに active 化する。active ではない resident は成果物作成、レビュー、Vault 更新を行わない。

bridge、commit、git-publisher、push、git-workspace-prep、save、Obsidian CLI、browser などの道具スキルは resident agent ではなく一時実行ツールとして扱う。ユーザーが明示した場合または Completion Gate / Branch Plan から必要になった場合だけ一時的に呼び出し、常駐 roster には入れない。
...
| `roster_scope` | `session` を既定とする |
| `chat_session_id` | 紐づくチャット/作業セッション ID |
| `project_id` | 紐づく Project ID または Vault project slug |
| `lifecycle_status` | `bootstrapping` / `ready` / `active` / `shutting_down` / `archived` / `failed` |
| `agent_id` | 正式ロール ID |
| `team` | `gate` / `tech` / `contents` / `business` / `infra` |
| `resident_status` | `resident` / `unavailable` |
| `activation_status` | `metadata_ready` / `idle` / `response_active` / `resetting` |
| `metadata_status` | roster metadata の作成状態 |
| `process_status` | `not_launched` / `process_ready` / `launch_failed` |
| `provider_status` | `not_started` / `deferred` / `provider_process_ready` / `provider_response_ready` |
...
| `effective_model` | transcript / session log で確認した実モデル |
| `execution_mode` | `agent` / `codex` / `chat` / `long-run` |
| `session_id` | Claude transcript sessionId、Codex session id など |
...
| `last_seen_at` | 最後に起動または稼働証跡を確認した時刻 |
...
- Team Role のモデル設定は `~/skills-repo/skills/infra-team-bootstrap/references/model-registry.md` を正本にする
