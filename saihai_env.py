"""Safe, stdlib-only Saihai environment loading.
...
class EnvError(RuntimeError):
    """Raised when Saihai environment configuration is unsafe or invalid."""
...
SCHEMA: dict[str, EnvField] = {
    ...
    "SAIHAI_ROOT": PATH,
    # Compatibility aliases: accepted for reads, never advertised in .env.example.
    "SAHAI_ROOT": PATH,
    ...
    "ITB_QUEUE_ROOT": PATH,
    "ITB_TASK_DETAIL_PATH": PATH,
    "ITB_REPO_ROOT": PATH,
    "ITB_ACTIVE_EXECUTION_CONTEXT_POINTER": PATH,
    "ITB_PROVIDER_ADD_DIRS": EnvField("path_list"),
    ...
    # Session/hook/test variables are known so process env remains compatible,
    # but are forbidden in .env.
    "ITB_RUNTIME": EnvField(),
    "ITB_STATE_ROOT": PATH,
    "ITB_BUILDER": PATH,
    "ITB_BOOTSTRAP_BUILDER": PATH,
    "ITB_PYTHON": PATH,
    "ITB_AGENT_CHILD": BOOL,
    "ITB_PARENT_SESSION_ID": EnvField(),
    "ITB_AGENT_ID": EnvField(),
    "ITB_FLOW_PHASE": EnvField(),
    "ITB_ORGANIZATION_INSTANCE_ID": EnvField(),
    "SAIHAI_VALIDATE_ALL_CHILD": BOOL,
}

INTERNAL_KEYS = frozenset({
    "ITB_RUNTIME", "ITB_STATE_ROOT", "ITB_BUILDER", "ITB_BOOTSTRAP_BUILDER",
    "ITB_PYTHON", "ITB_AGENT_CHILD", "ITB_PARENT_SESSION_ID", "ITB_AGENT_ID",
    "ITB_FLOW_PHASE", "ITB_ORGANIZATION_INSTANCE_ID", "SAIHAI_VALIDATE_ALL_CHILD",
})
ALIASES = {
...
def resolve_env_file(checkout_root: Path | None = None, environ: Mapping[str, str] | None = None) -> Path | None:
    ...
    if "SAIHAI_ROOT" in env and env["SAIHAI_ROOT"]:
        candidate = Path(env["SAIHAI_ROOT"]).expanduser() / ".env"
        ...
        for alias, canonical in ALIASES.items():
            if alias in target:
                ...
                if canonical not in target:
                    target[canonical] = (
                        os.path.join(target[alias], "skills")
                        if alias == "SKILLS_REPO_ROOT" and target[alias]
                        else target[alias]
                    )
