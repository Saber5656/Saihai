# Historical runtime environment audit

> Path classifications in this preserved audit are superseded by
> [environment-variable-inventory.md](environment-variable-inventory.md).
> This document retains the exhaustive runtime, hook, host, test, and tuning
> inventory from the original `.env` implementation. Non-path runtime settings
> remain file-configurable; runtime path/session entries are process-only.
