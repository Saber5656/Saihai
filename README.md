...
```sh
export SAIHAI_ORCH_STATE_ROOT=/tmp/frontdoor-state
STATE_ROOT="$SAIHAI_ORCH_STATE_ROOT"
...
default state root は `~/.codex/state/itb/frontdoor-orchestrator` です。`directory-path.env` または process environment の `SAIHAI_ORCH_STATE_ROOT` が設定されている場合は、その canonical root だけを使用します。`--state-root` は configured root との一致確認用であり、任意 path への切替には使えません。検証で disposable root を使う場合も、先に `SAIHAI_ORCH_STATE_ROOT=/tmp/frontdoor-state` を設定してください。
...
python3 scripts/configure_organization.py workflow-frontdoor-server \
  --state-root "$SAIHAI_ORCH_STATE_ROOT" \
  --host 127.0.0.1 \
  --port 8766
```
...
```sh
python3 scripts/configure_organization.py workflow-frontdoor --state-root "$SAIHAI_ORCH_STATE_ROOT" channel-token \
  --channel bridge
```
