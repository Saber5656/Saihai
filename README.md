...
```sh
STATE_ROOT="$HOME/.codex/state/itb/frontdoor-orchestrator"
...
default state root は `~/.codex/state/itb/frontdoor-orchestrator` です。primary checkout の `directory-path.env` に `SAIHAI_ORCH_STATE_ROOT` が設定されている場合は、その canonical root だけを使用します。この security-sensitive key に process environment override は適用しません。`--state-root` は configured root との一致確認用であり、任意 path への切替には使えません。
...
```sh
python3 scripts/configure_organization.py workflow-frontdoor --state-root "$STATE_ROOT" channel-token \
  --channel bridge
```
