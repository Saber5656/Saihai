...
Saihai は、プロンプトを実行権限として扱うのではなく、typed artifact、明示的な承認、durable state、監査可能な evidence を通じて AI エージェントの作業を実行するためのローカル orchestrator / organization runtime リポジトリです。
...
- Saihai bridgeが受理するのはtyped requestのsubmit、redacted projectionのread、outputのacknowledgeだけです。frontendが指定するauthoritative classification、approval、run作成、raw command/path、publication authorityは拒否します。このAPI制限だけでは、独立起動されたagentのambient authorityは除去されません。
- redacted projectionが決定論的な照合のために公開するのはidempotency digestだけで、raw keyは公開しません。child-thread / worker summaryはrequest、task、owner principal、checkoutのbindingがすべて一致する場合だけ表示し、binding欠落やlegacy unboundなsummaryは非表示にします。
- shipped enforcement targetは、root-ownedなzero-argument launcherから起動するrelease-pinned stock Codex CLIだけです。currentな`action_enforced` generationがある場合も、frontendのpositive pathはtyped submit 1件が`waiting_human`で停止するところまでです。capability、worker、run、provider dispatchなどのdownstream executionは作成しません。Codex App、IDE、direct Codex launchはclaimの対象外で、`ingress_enforced`もclaimしません。
- authority判定はcurrentなlive launcher processとcurrent deployment epochにbindされます。activate、rollback、uninstallはdeployment targetを変更する前に旧epochを失効させるため、以前のbyte列が復元されてもfresh commissioning / sealが必要です。
- frontendの`credential_access = denied`は、既知の2つのCodex auth path、dedicated `CODEX_HOME`、fixed inventoryにcredential-capable tool classがないことだけを対象にします。secretを含む可能性があるすべてのuser-readable fileが読めないというclaimではありません。
- host-owned executorは、currentな`action_enforced`と`managed_worker`の両generationが検証できる間だけ、approved work orderからcapabilityを導出し、pinnedかつboundedなCodex CLI workerを起動できます。workerには別管理のpolicy domainが必要です。v0.1.0はfrontend gatewayからworker domainへのautomatic cross-domain transportをshipしておらず、same rootfsでactiveな`managed_worker` generationをclaimすることもできません。
...
- runtime userのhomeは、そのuserが所有するcanonicalなnon-symlink directoryで、group / world writableではない
...
# configuration guide に示す8個の directory option を指定してから catalog を
# 検証する。SAIHAI_ROOT の default は primary checkout。
python3 scripts/setup_directory_paths.py --check
```

setup command は非破壊的で、owner-only file を書き込みます。process environment の値は、明示的に空にされた値も含めて、通常は catalog より優先されます。orchestrator state-root keyの`SAIHAI_ORCH_STATE_ROOT`だけは意図的な例外で、process environmentからoverrideできず、CLIで明示する`--state-root`もcatalog/defaultのcanonical rootと完全一致する必要があります。解決規則と復旧方法は [Local environment configuration](docs/configuration.md)、path audit の全体は [Directory path variable inventory](docs/environment-variable-inventory.md) を参照してください。
...
| 機能 | 現在の挙動 |
|---|---|
| Prompt classification | `scripts/configure_organization.py classify` と `/api/decide` は、`fast` / `strict` の execution mode と、`enabled` / `disabled` / `maintenance` の organization state を別々に返す。どの mode / state でも適用される task record と Vault record が必要 |
...
| Main-agent bridge | bridgeはrequestのsubmit、redacted projectionのread、outputのacknowledgeを受理し、authoritative classification、approval、run作成、adapter preparation、report pathを拒否する。projectionはidempotency digestだけを公開し、raw keyは公開しない。このAPI contractだけでは、独立起動されたagentのambient authorityは除去されない |
| Child-thread action gateway | `child-thread-create` が検証済みの issue-scoped child-worktree plan と結果を記録する。main-agent projectionにはcurrent requestとrequest、task、owner principal、checkoutのbindingがすべて一致するredacted summaryだけが含まれる |
| Scoped worker executor | executor contractとcommissioning scaffoldingは実装済みだが、currentなsame-rootfs external-mutation/git/credential factはnon-promotableなので`managed_worker`はsuppressされる。deriveとexecuteは両claimとper-capability repository/worktree bindingを再検証する。isolated worker domainのstronger evidenceだけがclaimをactivateでき、そのdomainへのautomatic transportはshipされていない |
...
| `classify` | optional な `--mode {fast,strict}` と、独立した `--organization-state {enabled,disabled,maintenance}` control を使って prompt を分類する |
...
| `bridge-retention-purge` | retention対象になったterminal promptをredactし、期限切れbridge index、acknowledgement、rate-limit record、rotated auditだけをpurgeする |
| `state-permission-repair` | private state modeをauditする。legacy modeを修復してdurable evidenceを書く場合だけ、local manual-operator pathから`--apply`を付ける |
...
### agentの起動に依存しないA′ frontend

portableなA-prime（`A′`）modelでは、Saihaiが全agent productの通常UI、authentication、session lifecycleを所有する必要はありません。ただし、各adapterは実際に強制できる具体的な境界を宣言する必要があります。bridgeからsubmitされたrequestについては、Saihaiがtyped requestと、その後のapproval、capability、side-effect、evidenceの各gateを所有します。全prompt entrypointがingress-enforcedだとはclaimしません。

machine-readable assurance contractは、次の4つの独立した状態を分けます。

| State | Claim |
|---|---|
| `advisory` | instructionまたはobservationだけで、機械的な保証はない |
| `ingress_enforced` | 宣言した全prompt entryがSaihaiへ到達するか停止する |
| `action_enforced` | 宣言した直接side-effect pathは拒否され、targetのtyped Saihai gatewayだけがpositive pathになる。worker executionを意味しない |
| `managed_worker` | Saihaiがbounded worker runtime、binary/config、environment、timeout、evidenceを所有し、各capabilityが実repository/worktreeを別途bindする |

Codexが最初の具体的なfrontend targetです。claimの対象は、root-owned Saihai launcherから起動したrelease-pinnedなstock Codex CLI 0.144.1 processだけです。Codex AppとIDE sessionは、current requirementsで禁止できないdynamic toolをapp-server clientが注入できるため、このclaimではunsupportedです。通常のhuman entrypointはzero-argument commandの`/usr/bin/sudo /usr/local/bin/saihai-codex-main-agent`です。standard supervisor sessionとshort-livedな固定commissioning sessionは別のrecordであり、相互に代用できません。targetは`action_enforced`であり、Saihaiは全Codex prompt entrypointが`ingress_enforced`だとはclaimしません。

bridge MCP serverが公開するserver-backed toolは`submit_request`、`read_projection`、`ack_output`の3つだけです。commissioningでのpositive observationは、stored requestが`waiting_human`になるtyped submit 1件だけで、capability、worker execution、run、provider evidence、report、marker changeはありません。review済みのread-only/internal Codex toolが見える場合はあり、`apply_patch`が見える場合も実write canaryで拒否されなければなりません。targetは、root-owned deploymentとCLI binary、machine-wide requirements、独立host observationが揃い、immutable generationがsealされてactive pointerに選択されるまでsuppressされます。変更可能なuser profileはdevelopment/routing defaultを提供するだけです。Claudeはadvisoryのままで、CursorとGrokは将来のtarget contractを記録しますが、candidate/unavailableとしてsuppressされたままです。

```sh
python3 organization/runtime/workflows/scripts/agent_integration_assurance.py report
```

Codex profile、必須のadministrator deployment、canary、worker policy-domainの制約は[main-agent action-enforcement runbook](docs/runbooks/main-agent-enforcement.md)を参照してください。最初のworkspace bindingは`Saber5656/Saihai`です。他repositoryはhost-owned bindingの追加とattestationが完了するまでfail closedします。

### Scoped worker backend

live scoped-worker backend は、host operator が以下の asset を手動設定するまで fail closed します。Saihai は key や credential を生成しません。

executor implementationはshippedですが、live `managed_worker` claimはsuppressされています。Codex 0.144.1ではsame-rootfsなlocal `git push`の絶対拒否を証明できません。currentなworker commissioningはevidenceを記録できても、`commission-seal`が`worker_denial_facts_not_promotable`でfail closedします。currentな`external_mutation`、`git_commit`、`git_push`、`credential_access` factは`result=fail`でhost observationもinconclusiveです。特に`workspace_profile_and_network_disabled_not_same_rootfs_isolation`と`dedicated_auth_deny_configured_not_mechanically_proven`は機械的なdeny evidenceではなく、明示的なnon-claimです。active worker claimには別のisolated policy domainで得たevidenceが必要です。shipped local action gatewayにはそのdomainへのautomatic transportがないため、current releaseは自動接続済みのend-to-end worker systemではありません。
...
| `SAIHAI_SCOPED_REPO_ROOT` | host-owned absolute repository path。default は Saihai repository root |
...
初期v1 capabilityがauthorizeするscopeはtask worktree全体だけです。subpath grant、commit、push、PR publication、credential access、worker-tool network、任意providerはauthorized capabilityの対象外です。same-rootfs hardeningはdefense in depthでありpromotable assuranceではありません。schemaやprobe recordだけからactiveなnative enforcementを推測してはいけません。固定されたCodex model control planeはhost transportであり、worker toolへのnetwork/provider grantではありません。capabilityの発行と実行はCLI subcommandではなく、credential-boundな`action_gateway` HTTP channelからだけ利用できます。
...
| `GET /api/decide?prompt=<text>` | `fast` / `strict` mode と、独立した `enabled` / `disabled` / `maintenance` organization state を返す decision |
...
| Main-agent deployment | `/Library/Application Support/Saihai/Manifests/codex-main-agent.deployment.json`, `/Library/Application Support/Saihai/Config/codex-main-agent.runtime.json` | root-ownedなrelease、wrapper、requirements、profile、instruction、workspace、policy-domain binding |
| Private commissioning record | `/Library/Application Support/Saihai/Assurance/commissioning/<profile_id>/<commissioning_id>.json` | root-onlyなsingle-use commissioning lifecycle record |
| Deployment epoch | `/Library/Application Support/Saihai/Assurance/epochs/<profile_id>.json` | root-ownedなtransition / revocation state。deployment transitionごとに古いgeneration bindingを無効化する |
| Immutable assurance generation | `/Library/Application Support/Saihai/Assurance/generations/<profile_id>/<generation_id>/` | generation manifest、attestation、observer record、observation、evidence、固定marker、canary。model proseはauthorityではない |
| Active generation pointer | `/Library/Application Support/Saihai/Assurance/active/<profile_id>.json` | 各action gateが再検証する、atomically選択されたgeneration |
| Launch-session record | `/Library/Application Support/Saihai/Assurance/launch-sessions/<session_id>.json`, `/Library/Application Support/Saihai/Assurance/commissioning-launches/<session_id>.json` | root-ownedなstandard / commissioning launch binding。2つのsession typeは相互に代用できない |

`epochs`を含むpublic assurance directory/fileはexact `0755`/`0644`、privateな`commissioning/**`はexact `0700`/`0600`、lock fileは`0600`です。ownerまたはmodeがdriftすると、対象claimはsuppressされます。
